...
class FilmTrackExport extends React.Component {
   constructor(props) {
     super(props)
   }
   state = {
   ...
   filmTrackExport = () => {
     ...
     let headersRow = [
      ...
      // --------------------------------------------------------------------------------------------------
        let descriptionString = exportTools.parseData(this.keywordParse(row.cue_desc).join(', ')).join(', ');
      ...
      // --------------------------------------------------------------------------------------------------
        let composerArray = this.props.selectedComposers.filter(composer =>
      ...
      // --------------------------------------------------------------------------------------------------
        ...
        let newRow = [
          ...
          // Composer1Type
          this.composerOrArrangerParse(row),
          // Composer2
          composerArray[1]
          ...
          // Composer2Type
          composerArray[1] ? this.composerOrArrangerParse(row) : '',
          // composer3
          composerArray[2]
          ...
          // Composer3Type
          composerArray[2] ? this.composerOrArrangerParse(row) : '',
          // composer4
          composerArray[3]
          ...
          // Composer4Type
          composerArray[3] ? this.composerOrArrangerParse(row) : '',
          // composer5
          composerArray[4]
          ...
          // Composer5Type
          composerArray[4] ? this.composerOrArrangerParse(row) : '',
          // composer6
          composerArray[5]
          ...
          // Composer6Type
          composerArray[5] ? this.composerOrArrangerParse(row) : '',
          // Publisher1
          pubMergeArray[0].publisher_name,
          // Publisher1Type
          'Publisher',
          // Publisher1Affiliation
          this.publisherProParse(pubMergeArray[0].publisher_name),
          // Publisher1Split
          ...
          pubMergeArray[1] ? this.publisherProParse(pubMergeArray[1].publisher_name) : '',
          // Publisher2Split
          ...
          pubMergeArray[2] ? this.publisherProParse(pubMergeArray[2].publisher_name) : '',
          // Publisher3Split
          ...
          // Version
          this.versionParse(row),
          // Instruments
          instrumentsString,
          // Tempo
          this.tempoParse(row.tempo_id),
          // Genre
          this.genreParse(row),
          // Categories
          this.catParse(row),
      ...
      () => { // done()
        ...
        this.props.downloadCompletedChecker();
        exportTools.generateDownload(this.state.xlsData.join('\n'), `DLM_${this.props.releaseFilter.label + "_"}FILM_TRACK_EXPORT_${moment().format('YYYY.MM.DD-HH_mm_ss')}.xls`);
      })
    }
  ...
  publisherProParse = (publisher_name) => {
