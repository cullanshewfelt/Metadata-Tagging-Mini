...
class AlterKExport extends React.Component {
   constructor(props) {
     super(props)
   }
   state = {
   ...
   let progressCount = 0;
    ...
    exportTools.asyncExport(filteredLibrary, filteredLibrary.length, (row) => {
     ...
     () => { // done()
       ...
       this.props.downloadCompletedChecker();
       exportTools.generateDownload(this.state.xlsData.join('\n'), `DLM_AlterK_${this.props.releaseFilter.label + "_"}${this.props.inclusive ? 'INC_' : ''}UPDATE_METADATA_${moment().format('YYYY.MM.DD-HH_mm_ss')}.xls`);
    })
   }
  render(){
