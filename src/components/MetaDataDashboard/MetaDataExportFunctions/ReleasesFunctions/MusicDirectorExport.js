...
class MusicDirectorExport extends React.Component {
   constructor(props) {
     super(props)
   }
   state = {
  ...
  musicDirectorExport = () => {
   ...
   () => { // done()
     ...
     this.props.downloadCompletedChecker();
     exportTools.generateDownload(this.state.xlsData.join('\n'), `DLM_${this.props.releaseFilter.label + "_"}MUSIC_DIRECTOR_BI_EXPORT_${moment().format('YYYY.MM.DD-HH_mm_ss')}.xls`);
   })
  }
