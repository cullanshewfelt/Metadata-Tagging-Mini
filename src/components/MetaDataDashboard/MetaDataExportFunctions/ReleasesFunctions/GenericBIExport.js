...
class GenericBIExport extends React.Component {
   constructor(props) {
     super(props)
   }
   state = {
  ...
  genericBIExport = () => {
   ...
   () => { // done()
     ...
     this.props.downloadCompletedChecker();
     exportTools.generateDownload(this.state.xlsData.join('\n'), `DLM_${this.props.releaseFilter.label + "_"}GENERIC_BI_EXPORT_${moment().format('YYYY.MM.DD-HH_mm_ss')}.xls`);
   })
  }
  render(){
      ...
      <a onClick={(() =>
        releaseFilter === 147 || this.props.selectedLibrary.libraryName === 'independent-artists'
          ...
          : this.props.inclusive || releaseFilter.label === 'All'
            ? exportTools.exportError('Please Unselect Inclusive.')
            : this.genericBIExport())
      } className={
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(GenericBIExport));
