...
class BmatExport extends React.Component {
   constructor(props) {
     super(props)
   }
   state = {
  ...
  bmatExport = () => {
    ...
    // console.log(54, numbersAsString['1'])
    exportTools.asyncExport(filteredLibrary, filteredLibrary.length, (row) => {
      ...
      let renamedTitle = /v\d{1,2}/.test(row.cue_title)
        ? row.cue_title.replace(/v\d{1,2}/, /v\d{1,2}/.exec(row.cue_title)[0])
        : `DLM ${row.cue_title}`;
      /[0-9]/g.test(renamedTitle)
        ...
        : renamedTitle;
      let thisRel = this.props.batchesDropDown.filter(batch =>
      ...
      let newRow = [
          ...
          `${genre}`,
          // GEN:1:SubGenre
          `${this.props.selectedStyles.filter(styles =>
            styles.style_id === row.style_id).map(style =>
              style.style_name)}`,
          // GEN:2:Genre
          '',
    ...
    () => { // done()
      ...
      this.props.downloadCompletedChecker();
      exportTools.generateDownload(this.state.csvData.join('\n'), `DLM_${this.props.releaseFilter.label + "_"}BMAT_EXPORT_${moment().format('YYYY.MM.DD-HH_mm_ss')}.txt`);
    })
  }
  render(){
