...
class SourceAudioExport extends React.Component {
   constructor(props) {
     super(props)
   }
   state = {
    ...
    exportTools.asyncExport(filteredLibrary, filteredLibrary.length, (row) => {
      ...
      if(row.cat_id === 1 || row.cat_id === 3 || row.cat_id === 4 || row.cat_id === 14 || row.cat_id === 15){
        switch(row.cat_id){
          // BUMPERS - TRANSITIONS  ***************************************************************
          case 1:
            parsedGenre = `Stingers (${this.props.selectedStyles.filter(styles =>
          ...
          // DRAMA  ***************************************************************
          case 3:
            parsedGenre = `Drama (${this.props.selectedStyles.filter(styles =>
              styles.style_id === row.style_id).map(style =>
                style.style_name)})`;
          ...
          // DRAMA ORCHESTRAL  ***************************************************************
          case 4:
            parsedGenre = `Orchestral (${this.props.selectedStyles.filter(styles =>
              styles.style_id === row.style_id).map(style =>
                style.style_name)})`;
            break;
          // WORLD MUSIC  ***************************************************************
          case 14:
            parsedGenre = `World Music (${this.props.selectedStyles.filter(styles =>
              styles.style_id === row.style_id).map(style =>
                style.style_name)})`;
          ...
          // WORLD POP  ***************************************************************
          case 15:
            parsedGenre = `World Pop (${this.props.selectedStyles.filter(styles =>
              styles.style_id === row.style_id).map(style =>
                style.style_name)})`;
            break;
        }
      ...
      switch(parseInt(row.style_id)){
        ...
        case 128:
          ...
          break;
        default:
          parsedGenre = this.props.selectedStyles.filter(styles =>
            styles.style_id === row.style_id).map(style =>
              style.style_name)[0]
      }
    }
      ...
      let newRow = [
          ...
          // Album
          `${this.props.selectedCategories.filter(category =>
             category.cat_id === row.cat_id).map(cat =>
               cat.cat_name)}, ${this.props.selectedStyles.filter(styles =>
                 styles.style_id === row.style_id).map(style =>
                   style.style_name)} Vol. ${this.props.releaseFilter.label}`,
          // Album Code
          `DLM-BI-${row.style_id.toString().padStart(3, 0)}-${this.props.releaseFilter.label}`,
          // Album Description
          parsedGenre,
          // Album Genre
          parsedGenre,
          ...
          '',
          // Release
          this.props.releaseFilter.label
     ];
     // console.log(newRow)
     progressCount ++;
     ...
     this.props.downloadCompletedChecker();
     exportTools.generateDownload(this.state.csvData.join('\n'), `DLM_${this.props.releaseFilter.label + "_"}${this.props.inclusive ? 'INC_' : ''}Source_Audio_${moment().format('YYYY.MM.DD-HH_mm_ss')}.csv`);
   })
}
  render(){
    ...
    return (
      <a onClick={(() =>
          ...
          ? exportTools.exportError('We Don\'t Send Our Independent Artists Catalog To SourceAudio')
          // : this.props.inclusive                                                     // take these lines out to prevent inclusive exports
          // ? exportTools.exportError('You Don\'t Export Inclusive, Just the Batch') // take these lines out to prevent inclusive exports
          : this.sourceAudioExport())
      } className={
