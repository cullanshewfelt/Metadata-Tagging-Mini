...
class ModoocomIAExport extends React.Component {
   constructor(props) {
     super(props)
   }
   state = {
  ...
  modoocomIAExport = () => {
   ...
   () => { // done()
     ...
     this.props.downloadCompletedChecker();
     exportTools.generateDownload(this.state.xlsData.join('\n'), `DLM_${this.props.releaseFilter.label + "_"}MODOOCOM_IA_EXPORT_${moment().format('YYYY.MM.DD-HH_mm_ss')}.xls`);
   })
  }
