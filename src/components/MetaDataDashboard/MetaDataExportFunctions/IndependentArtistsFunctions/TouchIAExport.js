...
class TouchIAExport extends React.Component {
   constructor(props) {
     super(props)
   }
   state = {
  ...
  // ******************************************************************************************
  //  TOUCH IA EXPORT FUNCTION
  // ******************************************************************************************

  //  seems finished, diffCheck, refactor
  ...
  touchExport = () => {
    ...
    let filteredLibrary = this.props.selectedLibrary.library.filter(cue =>
        this.props.inclusive
          ? cue.rel_id <= this.props.releaseFilter.value && (cue.cue_status === 'Active' || 'Instrumental_Active')
          : this.props.releaseFilter.label === 'All'
          ? cue.cue_status === 'Active' || 'Instrumental_Active'
          ...
          ? cue.rel_id <= releasesArray[releasesArray.length - 1] && (cue.cue_status === 'Active' || 'Instrumental_Active')
          : cue.rel_id === this.props.releaseFilter.value && (cue.cue_status === 'Active' || 'Instrumental_Active')
   ...
   () => { // done()
     ...
     this.props.downloadCompletedChecker();
     exportTools.generateDownload(this.state.xlsData.join('\n'), `DLM_${this.props.releaseFilter.label + "_"}TOUCH_IA_EXPORT_${moment().format('YYYY.MM.DD-HH_mm_ss')}.xls`);
   })
  }
  render(){
