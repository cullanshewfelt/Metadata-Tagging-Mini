...
const PreludioExport = (props) => {
  ...
  useEffect(() => {
    let downloadLink = `DLM_${releaseFilter.label + "_"}PRELUDIO_EXPORT_${moment().format('YYYY.MM.DD-HH_mm_ss')}.txt`;
    progress === 1
  ...
  const preludioExport = () => {
    ...
    // --------------------------------------------------------------------------------------------------
      ...
      let newRow = [
        ...
        atmosphereParse(row),
        // Genres
        isBG ? genreBIParse(row) : genreIAParse(row),
        // Products
        isBG ? productBIParse(row) : productIAParse(row),
        // Rhythm
        row.tempo_id === 8 ? tempos.filter(tempo =>
